...
import { applyProviderTemplate, testServiceConnection } from './providerTemplates';
...
export function SettingsPanel({ preferences, assets, secrets, onChange, onSecretChange, onAssetChange, onClose }: { preferences: Preferences; assets: AssetState; secrets: Record<ServiceId, string>; onChange(value: Preferences): void; onSecretChange(id: ServiceId, value: string): void; onAssetChange(kind: LocalAssetKind, value: AssetState[LocalAssetKind]): void; onClose(): void }) {
  ...
  return <div className="settings-backdrop" onMouseDown={onClose}><section className="settings-panel" aria-label="设置中心" onMouseDown={event => event.stopPropagation()}>
    ...
    <div className="settings-body"><nav aria-label="设置分类">{tabs.map(item => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}><i>{item.glyph}</i>{item.label}</button>)}</nav><div className="settings-content">
      ...
      {tab === 'ai' && <>{(['chat', 'vision', 'speech'] as ServiceId[]).map(id => <ServiceCard key={id} id={id} value={preferences.services[id]} apiKey={secrets[id]} onApiKey={value => onSecretChange(id, value)} update={patch => updateService(id, patch)}/>)}</>}
